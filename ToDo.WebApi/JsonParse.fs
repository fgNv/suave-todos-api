...
module Tag =
    ...
    let private deserializeCreateTagCommand' =            
         json {
             let! id = Json.read "id"
             let! name = Json.read "name"
             let! creatorId = Json.read "creatorId"
             return { id = id; name = name; creatorId = creatorId }
         }

    let deserializeCreateTagCommand = deserializeJson deserializeCreateTagCommand'
